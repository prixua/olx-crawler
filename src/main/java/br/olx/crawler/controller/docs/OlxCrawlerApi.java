...
public interface OlxCrawlerApi {
    ...
    /**
     * Envia uma mensagem via WhatsApp para o número fixo.
     * Endpoint aciona o serviço externo de WhatsApp e retorna status do envio.
     */
    @Operation(
            summary = "Enviar mensagem via WhatsApp",
            description = "Envia uma mensagem via integração com API externa.",
    ...
    @ApiResponses(value = {
            @ApiResponse(
                    ...
                    content = @Content(
                            mediaType = "application/json",
                            schema = @Schema(example = "Mensagem enviada com sucesso")
                    )
            ),
            @ApiResponse(
                    responseCode = "500",
                    ...
                    content = @Content(
                            mediaType = "application/json",
                            schema = @Schema(example = "Falha ao enviar mensagem")
                    )
            )
    })
    Mono<ResponseEntity<String>> sendMessage();
}
